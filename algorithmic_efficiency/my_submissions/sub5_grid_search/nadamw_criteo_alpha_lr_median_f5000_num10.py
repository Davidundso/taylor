...
def update_params(workload: spec.Workload,
  ...
  if (global_step < initial_step) or ((adjusted_step % comp_alphas_each >= num_consec_alphas) and (global_step > 2000)):
    hyperparameters = HPARAMS
  ...
  log_dir = os.path.expandvars("$WORK/cluster_experiments/time_f5000_num10")
  ...
  if (adjusted_step % comp_alphas_each == 0) or (global_step == 0):
    alpha_values = []
  ...
  if (adjusted_step % comp_alphas_each == num_consec_alphas - 1 and global_step > comp_alphas_each) or global_step == 2009:
