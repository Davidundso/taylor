...
def update_params(workload: spec.Workload,
  ...
  # log the learning rate in a separate file
  ...
  log_dir = os.path.expandvars("$WORK/cluster_experiments/normal_run")

  # Ensure the directory exists
  os.makedirs(log_dir, exist_ok=True)

  # Construct the full path to the log file
  log_file_path = os.path.join(log_dir, 'learning_rate_log.csv')

  log_data = [global_step, current_lr, loss.item(), grad_norm.item()]
  ...
  try:
      with open(log_file_path, 'x') as log_file:  # Open in exclusive creation mode
          ...
          writer.writerow(["Global step", "Learning rate", "Loss", "Gradient norm"
                          ])  # Write header
...
def get_batch_size(workload_name):
  ...
  if workload_name == 'criteo1tb':
    return int(262_144/8)  # divide by 8 for using only one instead of 8 a100 GPUs (compared to AlgoPerf comptetion)
