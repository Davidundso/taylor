...
def update_params(workload: spec.Workload,
  ...
  Data_b1 = [(inputs1, targets1)] # remove 'view(-1, 1)' for mnist
  ...
  Data_b2 = [(inputs2, targets2)] # remove 'view(-1, 1)' for mnist
  ...
  try:
      with open(log_file_path, 'x') as log_file:  # Open in exclusive creation mode
          ...
          writer.writerow(["Global step", "alpha1 unbiased (GGN,g B1; d B2)", "alpha2 unbiased (GGN,g B2; d B1)", "alpha B1 biased", "alpha B2 biased", "Learning rate"
                           ])  # Write header
  ...
  log_data = [global_step, alpha_star1.item()*current_lr, alpha_star2.item()*current_lr, alpha_star_b1.item()*current_lr, alpha_star_b2.item()*current_lr, current_lr]
  ...
  try:
      with open(log_file_path, 'x') as log_file:  # Open in exclusive creation mode
          writer = csv.writer(log_file)
          writer.writerow(["Global step", "alpha1 unbiased * lr (GGN,g B1; d B2)", "alpha2 unbiased * lr (GGN,g B2; d B1)", "alpha B1 biased * lr", "alpha B2 biased * lr", "Learning rate"
                           ])  # Write header
  except FileExistsError:
      pass
  ...
  cosine_2 = torch.dot(gradients_b2, d_unnormalized_b1) / (gradients_b2.norm() * d_unnormalized_b1.norm())
  ...
  log_data = [global_step, 
              alpha_star1.item(), dg1.item(), dGGNd_1.item(), cosine_1.item(),
                alpha_star2.item(), dg2.item(), dGGNd_2.item(), cosine_2.item(),
  ...
  try:
      with open(log_file_path, 'x') as log_file:  # Open in exclusive creation mode
          ...
          writer.writerow(["Global step",
            ...
            "alpha2 unbiased", "dg2 (d B1, g B2)", "dGGNd2 (d B1, GGN B2)", "cosine2 (d B1, g B2)",
            "alpha1 biased", "dg1 biased", "dGGNd1 biased", "cosine1 biased",
            "alpha2 biased", "dg2 biased", "dGGNd2 biased", "cosine2 biased",
  ...
  log_data = [global_step, gradients_norm_b1.item(), gradients_norm_b2.item(), torch.norm(d_unnormalized_b1, 2).item(), torch.norm(d_unnormalized_b2, 2).item(), loss1.item(), loss2.item()]

  # Check if the file exists and write a header if needed
  try:
      with open(log_file_path, 'x') as log_file:  # Open in exclusive creation mode
          writer = csv.writer(log_file)
          writer.writerow(["Global step", "Gradient norm B1", "Gradient norm B2", "d norm B1", "d norm B2", "Loss B1", "Loss B2"
                           ])  # Write header
  except FileExistsError:
      pass
