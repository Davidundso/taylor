...
def update_params(workload: spec.Workload,
  ...
  if global_step == 1:
     params_without_grad = [param for param in current_model.parameters() if not param.requires_grad]
     print(f'Number of parameters without grad: {len(params_without_grad)}')
  ...
  # Update the parameters after the first half
  optimizer_state['optimizer'].step()
  # do not step the scheduler here, the step will be reversed

  theta_1_b1 = parameters_to_vector([param.detach().clone() for param in current_param_container.parameters() if param.requires_grad]).cpu()  

  d_unnormalized_b1 = theta_1_b1 - theta_0_b1

  d_unnormalized_b1 = d_unnormalized_b1.to('cpu')  # Move to CPU if necessary

  # undo the step that was done in the first half by setting the parameters to the values before the step
  ...
  # check what optimizer state dict holds
  if global_step == 1:
  ...
  if global_step <= 100 or global_step % 500 == 0:
    ...
    for param_group_before, param_group_after in zip(optimizer_state_before['param_groups'], optimizer_state_after['param_groups']):
        for key in param_group_before:
            value_before = param_group_before[key]
            value_after = param_group_after[key]

            # Check if the values are tensors and compare them
            if isinstance(value_before, torch.Tensor) and isinstance(value_after, torch.Tensor):
                if not torch.equal(value_before, value_after):
                    print(f"Mismatch detected in tensor parameter group for {key}")
                    state_restored = False
                    break
            # Check if the values are floats and compare them
            elif isinstance(value_before, float) and isinstance(value_after, float):
                if value_before != value_after:
                    print(f"Mismatch detected in float parameter group for {key}")
                    state_restored = False
                    break
    ...
    if state_restored:
      ...
      print("Error: Optimizer state was not restored properly.")
  ...
  #GGN_b2 = GGNLinearOperator(current_model, loss_fn, params_list, Data_b2) for time reasons, we do not compute the GGN for the second half
  ...
  if print_bool:
    print(f'alpha_star1: {alpha_star1}, alpha_star_b1: {alpha_star_b1}')
  ...
  log_dir = os.path.expandvars("/home/suckrowd/Documents/experiments_algoPerf/mnist050125_alpha_as_lr")
  ...
  log_data = [global_step, alpha_star1.item(), alpha_star_b1.item(), current_lr]
  ...
  try:
      with open(log_file_path, 'x') as log_file:  # Open in exclusive creation mode
          ...
          writer.writerow(["Global step", "alpha1 unbiased (GGN,g B1; d B2)", "alpha B1 biased", "Learning rate"
                           ])  # Write header
  ...
  log_data = [global_step, alpha_star1.item()*current_lr, alpha_star_b1.item()*current_lr, current_lr]
  ...
  try:
      with open(log_file_path, 'x') as log_file:  # Open in exclusive creation mode
          writer = csv.writer(log_file)
          writer.writerow(["Global step", "alpha1 unbiased * lr (GGN,g B1; d B2)",  "alpha B1 biased * lr",  "Learning rate"
                           ])  # Write header
  except FileExistsError:
      pass
  ...
  cosine_b1 = torch.dot(gradients_b1, d_unnormalized_b1) / (gradients_b1.norm() * d_unnormalized_b1.norm())
  ...
  log_data = [global_step, 
              ...
              alpha_star_b1.item(), dg_b1.item(), dGGNd_b1.item(), cosine_b1.item(),
                  current_lr]
  ...
  try:
      with open(log_file_path, 'x') as log_file:  # Open in exclusive creation mode
          ...
          writer.writerow(["Global step",
            ...
            "alpha1 biased", "dg1 biased", "dGGNd1 biased", "cosine1 biased",
  ...
  log_data = [global_step, gradients_norm_b1.item(), torch.norm(d_unnormalized_b1, 2).item(), torch.norm(d_unnormalized_b2, 2).item(), loss1.item(), loss2.item()]

  # Check if the file exists and write a header if needed
  try:
      with open(log_file_path, 'x') as log_file:  # Open in exclusive creation mode
          writer = csv.writer(log_file)
          writer.writerow(["Global step", "Gradient norm B1", "d norm B1", "d norm B2", "Loss B1", "Loss B2"
                           ])  # Write header
  except FileExistsError:
      pass
  ...
  if global_step % 50 == 49:
    ...
    for param_group in optimizer_state['optimizer'].param_groups:
                param_group['lr'] = averaged_alpha_star1
