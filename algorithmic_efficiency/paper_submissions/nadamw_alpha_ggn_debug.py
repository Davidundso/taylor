...
""" Attempt to use the GGN to compute the alpha for the NAdamW optimizer and to generalize the arguments such
that they work across all workloads. """
...
def update_params(workload: spec.Workload,
  ...
  def loss_func(logits, labels):
      return workload.loss_fn(
          label_batch=labels,
          logits_batch=logits
      )['summed']

  params = list(current_param_container.parameters())

  Data_b1 = [(inputs1.to(device), targets1.view(-1,1).to(device))]

  def batch_size_fn(X):
      return X.shape[0] if isinstance(X, torch.Tensor) else X['inputs'].shape[0]

  ggn_op = GGNLinearOperator(
      model_func=current_model,
      loss_func=loss_func,
      params=params,
      data=Data_b1,
