...
def update_params(workload: spec.Workload,
  ...
  assert inputs1.device == device, f"inputs1 on {inputs1.device}, expected {device}"
  assert targets1.device == device, f"targets1 on {targets1.device}, expected {device}"
  inputs1 = inputs1.to(device)
  targets1 = targets1.to(device)
  input_tensor = torch.cat([inputs1, targets1], dim=1)

  # Dummy class indices
  dummy_target = torch.full((16, 256), -100, dtype=torch.long, device=device)
  Data_b1 = [(input_tensor, dummy_target)]
  ...
  # debug on wmt: wrap model
  
  class ReshapedCrossEntropyLoss(nn.Module):
    def __init__(self, ignore_index=-100):
        super().__init__()
        self.loss_fn = nn.CrossEntropyLoss(ignore_index=ignore_index)
        self.reduction = self.loss_fn.reduction  # required by curvlinops
  ...
  def wrapped_model(X):
    ...
    out = current_model(src, tgt)
    print("Model output shape:", out.shape)
    return out



    # Extra safety: make sure model input is on the same device
    assert src.device == next(current_model.parameters()).device
    assert tgt.device == next(current_model.parameters()).device

    return current_model(src, tgt)

  loss_fn = ReshapedCrossEntropyLoss()
  GGN_b1 = GGNLinearOperator(wrapped_model, loss_fn, params_list, Data_b1)
  ...
  optimizer_state['optimizer'].zero_grad()
  # compute ggn
  Data_b2 = [(inputs2.to(device), targets2.to(device))] # remove 'view(-1, 1)' for mnist
