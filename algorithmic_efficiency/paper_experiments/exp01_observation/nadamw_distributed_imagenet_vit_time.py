...
def update_params(workload: spec.Workload,
  ...
  gradients_b1 = parameters_to_vector(param.grad for param in current_model.parameters() if param.grad is not None)
  ...
  if timing:
      time_grad_clip_b1 = time.time() - t_checkpoint
      t_checkpoint = time.time()

  params_list = [param for param in current_model.parameters() if param.requires_grad]
  theta_0_b2 = parameters_to_vector([param.detach().clone() for param in params_list])
  ...
  if timing:
      time_grad_clip_b2 = time.time() - t_checkpoint
      t_checkpoint = time.time()
